...
When depositing a validator, a withdrawal address is also provided. This address can be used to identify the entity that operates the validator, since in most cases, entities use the same withdrawal address for all of their validators.

This tool is used for tagging validators shown in [migalabs.io](https://migalabs.io).
...
We use the following definition of a whale: unidentified depositor/withdrawal address shared between at least [`WHALE_THRESHOLD`](https://github.com/migalabs/eth-pokhar?tab=readme-ov-file#identify) validators. The tool will tag the whale with the `whale_0x....` tag (first 4 characters of the depositor/withdrawal address).

Note that the whale tagging process is done at the start of the identification process, so the whales will be tagged before any other identification process is done. This means that if a whale is identified as a pool/entity, it will be tagged with the `whale_0x....` tag and the pool/entity tag will override the whale tag, resulting in a possible whale of less than `WHALE_THRESHOLD` validators.
